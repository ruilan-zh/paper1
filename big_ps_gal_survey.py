...
if halo_type == "gal":
    ...
    if logMmax is not None:
        mask = (mhalo_tng > 10) & (sfr_tng > sfr_lim) & (mhalo_tng < logMmax)
    else:
        mask = (mhalo_tng > 10) & (sfr_tng > sfr_lim) & (mhalo_tng < 16)
    #logMmin = 11.9
    #logMmax = 12.1
    ...
    sfr_lim = 1
    if number_density:
        tng_data1 = np.loadtxt(f"{tng_dir}/nsubs_w_logsfr>{sfr_lim}.txt")
    ...
    if logMmax is not None:
        mask = (mhalo_tng1 > 10) & (sfr_tng1 > sfr_lim1) & (mhalo_tng1 < logMmax)
    else:
        mask = (mhalo_tng1 > 10) & (sfr_tng1 > sfr_lim1) & (mhalo_tng1 < 16)
...
import pickle
odir=f"ps_data/snap{snapnum}/mvir/gal_survey/logSFR{sfr_lim}/{halo_type}"
#odir=f"ps_data/snap{snapnum}/mvir/number_density/group"
...
if logMmax is not None:
    fname = f"{odir}/logMmax{logMmax:.1f}.pickle"
else:
    fname = f"{odir}/logMmin10.pickle"
#fname = f"{odir}/luminosity_weighting.pickle"
#fname = f"{odir}/logM{logMmin}-{logMmax}.pickle"
