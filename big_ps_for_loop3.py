...
if nfile==2:
    ...
    logM_range1="logM11-11.5"
    conc_split1="conc_proxy_split2"
    tng_data1 = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM0.1_snap40/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_range1}/{sfr_range}/{bh_range}/{conc_base}/{conc_split1}/seed{seed}{shuffle_name}.txt")

        #tng_data2 = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM0.1_snap40/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_range}/{sfr_range}/{bh_range}/{conc_split}/seed{seed}{shuffle_name}.txt")
    mhalo_tng1 = tng_data1[:,0]
    pos_tng1 = tng_data1[:,1:4]
    sfr_tng1 = tng_data1[:,4]
    mask1 = (mhalo_tng1 > 11) & (mhalo_tng1 < 11.5)
    #mask1 = (mhalo_tng1 < 11.5)
    #mask1 = (mhalo_tng1 > 15) 
    print(len(mhalo_tng1))
    print("mask1", np.sum(mask1))

    logM_range2="logM12-13.8"
    #logM_range2=""
    conc_split2="msat_sum_split4"
    tng_data2 = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM0.1_snap40/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_range2}/{sfr_range}/{bh_range}/{conc_base}/{conc_split2}/seed{seed}{shuffle_name}.txt")

    mhalo_tng2 = tng_data2[:,0]
    pos_tng2 = tng_data2[:,1:4]
    sfr_tng2 = tng_data2[:,4]
    mask2 = (mhalo_tng2 > 12) & (mhalo_tng2 < 13.8)
    logM_range2="logM12-13.8"
    #mask2 = mhalo_tng2 > 15
    #mask2 = (mhalo_tng2 < 13.8)

    #print(len(mhalo_tng2))
    #print("mask2", np.sum(mask2))
    #print("1+2:", np.sum(mask1)+np.sum(mask2))

    object2shuffle3="cent_sat"
    conc_split3_name="conc_proxy_split4_msat_sum_split4"

    logM_range3="logM11.5-12"
    conc_split31="conc_proxy_split4"
    tng_data31 = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM0.1_snap40/{changed_bins}/cent_ps/shuffle_cent/{logM_range3}/{sfr_range}/{bh_range}/{conc_base}/{conc_split31}/seed{seed}{shuffle_name}.txt")

    mhalo_tng31 = tng_data31[:,0]
    pos_tng31 = tng_data31[:,1:4]
    sfr_tng31 = tng_data31[:,4]
    
    conc_split32="msat_sum_split4"
    tng_data32 = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM0.1_snap40/{changed_bins}/sat_ps/shuffle_sat/{logM_range3}/{sfr_range}/{bh_range}/{conc_base}/{conc_split32}/seed{seed}{shuffle_name}.txt")

    mhalo_tng32 = tng_data32[:,0]
    pos_tng32 = tng_data32[:,1:4]
    sfr_tng32 = tng_data32[:,4]
    
    mhalo_tng3 = np.concatenate((mhalo_tng31, mhalo_tng32))
    pos_tng3 = np.concatenate((pos_tng31, pos_tng32))
    sfr_tng3 = np.concatenate((sfr_tng31, sfr_tng32))
    mask3 = (mhalo_tng3 > 11.5) & (mhalo_tng3 < 12)
    #mask3 = (mhalo_tng3 > 11.5) 
    #mask3 = (mhalo_tng3 > 15) 



    logM_base = "logM11.5-12" # random one - not shuffled outside of this range 

    tng_data_base = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM0.1_snap40/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_base}/seed{seed}{shuffle_name}.txt")
    mhalo_tng_base = tng_data_base[:,0]
    pos_tng_base = tng_data_base[:,1:4]
    sfr_tng_base = tng_data_base[:,4]
    #mask_base = (mhalo_tng_base < 11) | (mhalo_tng_base > 13) | ((mhalo_tng_base > 11.5) & (mhalo_tng_base < 12))
    mask_base = (mhalo_tng_base < 11) | (mhalo_tng_base > 13.8) 
    #mask_base =  (mhalo_tng_base > 15) 
    print("base2", np.sum(mask_base))
    print("base difference", len(mhalo_tng_base)-np.sum(mask_base))
    #assert len(mhalo_tng_base)-np.sum(mask_base) == np.sum(mask1)+np.sum(mask2)

    mhalo_tng = np.concatenate((mhalo_tng1[mask1], mhalo_tng2[mask2],mhalo_tng3[mask3],mhalo_tng_base[mask_base]))
    pos_tng = np.concatenate((pos_tng1[mask1], pos_tng2[mask2],pos_tng3[mask3],pos_tng_base[mask_base]))
    sfr_tng = np.concatenate((sfr_tng1[mask1], sfr_tng2[mask2],sfr_tng3[mask3],sfr_tng_base[mask_base]))
    
    #mhalo_tng = np.concatenate((mhalo_tng2[mask2],mhalo_tng_base[mask_base]))
    #pos_tng = np.concatenate(( pos_tng2[mask2], pos_tng_base[mask_base]))
    #sfr_tng = np.concatenate((sfr_tng2[mask2], sfr_tng_base[mask_base]))

    print("New length:", len(mhalo_tng))
...
#odir = f"ps_data/snap{snapnum}/mvir/shuffled/logM10_dlogM0.1/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle},{object2shuffle3}/shuffle_all/{logM_range1},{logM_range2},{logM_range3}/{conc_split1},{conc_split2},{conc_split3_name}"
odir = f"ps_data/snap{snapnum}/mvir/shuffled/logM10_dlogM0.1/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle},{object2shuffle3}/logM11-13.8/{logM_range1},{logM_range2},{logM_range3}/{conc_split1},{conc_split2},{conc_split3_name}"
#odir = f"ps_data/snap{snapnum}/mvir/shuffled/logM10_dlogM0.1/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/shuffle_all/{logM_range2}/{conc_split2}"
#odir = f"ps_data/snap{snapnum}/mvir/shuffled/logM10_dlogM0.1/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle},{object2shuffle3}/shuffle_in_range_only/logM11-11.5,logM11.5-12/{conc_split1},{conc_split3_name}"
#odir = f"ps_data/snap{snapnum}/mvir/shuffled/logM10_dlogM0.1/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_range1}_{logM_range2}_{logM_range3}/{conc_split1}_{conc_split2}_{conc_split3_name}"
fname = f"{odir}/seed{seed}{shuffle_name}.pickle"

#odir = f"ps_data/snap{snapnum}/mvir"
#fname = f"{odir}/ps-intensity-{ps_type}_{logM_range}.pickle"
