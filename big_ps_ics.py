...
tng_dir = "/cosma7/data/dp004/dc-zhan5/TNG300-3/ics"
fname = f"{tng_dir}/snap_ics.hdf5"
"""
...
import h5py
f1 = h5py.File(fname,'r')
dm_coords = f1["PartType1"]["Coordinates"]

nhalo = len(dm_coords)
halocat = np.empty(nhalo, dtype=[("Position", ("f8", 3))])

halocat["Position"] = np.array(dm_coords)/1e3
ps_dict = {
    "Nhalo": nhalo,
    "halocat": halocat
}
...
if interp == "nearest":
    ...
    #mesh = arraycat.to_mesh(resampler=interp, BoxSize=boxsize, Nmesh=Nmesh, weight=weight, compensated=True, interlaced=True, position="PartType1/Coordinates")
