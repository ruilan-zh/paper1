...
if nfile==2:
    ...
    #tng_data1 = np.loadtxt(f"../nbodykit-ps/halo_exclusion/random_seed2.txt")
    tng_data1 = np.loadtxt(f"/cosma7/data/dp004/dc-zhan5/shuffled/logM10_dlogM{dlogM}_snap40/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_range}/{sfr_range}/{bh_range}/{conc_split}/seed{seed}{shuffle_name}.txt")
    mhalo_tng1 = tng_data1[:,0]
    pos_tng1 = tng_data1[:,1:4]
    sfr_tng1 = tng_data1[:,4]
...
pixsize_cm = pixlen_mpc_h * mpc /h
print("New pixlen:", pixlen_mpc_h, "cm")
pixlen_rad = pixlen_cm / (d_A_cm * (1+z))
pixlen = pixlen_rad * 360 / (2 * np.pi)
pixsize = pixlen * pixlen
pixsize_sr = pixsize / (180/np.pi)**2 # convert from deg2 to sr
...
odir = f"ps_data/snap{snapnum}/mvir/shuffled/logM10_dlogM{dlogM}/{changed_bins}/{ps_type}_ps/shuffle_{object2shuffle}/{logM_range}/{sfr_range}/{bh_range}/{conc_split}"
#odir = f"ps_data/snap{snapnum}/mvir/tests/random"
fname = f"{odir}/seed{seed}{shuffle_name}.pickle"

#odir = f"ps_data/snap{snapnum}/mvir"
#fname = f"{odir}/ps-intensity-sum_{logM_range}.pickle"
