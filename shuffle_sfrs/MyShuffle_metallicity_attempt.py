...
class Shuffler:
    def __init__(self, ps_type, object2shuffle, snapnum=40, boxsize=205, include_bh=False, include_halo_structure=False, include_msat_sum=False, include_conc_proxy=False, include_metallicity=False):
        ...
        self.include_metallicity = include_metallicity
    ...
    def read_tng(self):
        ...
        if self.include_metallicity:
            ihalo_dir = f"{base_dir}/ihalo/SubhaloGasMetallicitySfrWeighted"
        ...
        if self.ps_type == "group":
            ...
            if self.include_metallicity:
                metallicity_tng1 = tng_data1[:,7][mask_min]
            ...
            if self.include_metallicity:
                metallicity_tng2 = tng_data2[:,7][mask_min2]
            ...
            if self.include_metallicity:
                metallicity_main = np.concatenate((metallicity_tng1,metallicity_tng2))
            ...
            if self.ps_type != "cent":
                ...
                if self.include_metallicity:
                    metallicity_tng_sat_abs = tng_data_sat_abs[:,7][mask_min]
                    self.metallicity_tng_sat_abs = metallicity_tng_sat_abs
                ...
                if self.object2shuffle != "cent":
                    ...
                    if self.include_metallicity:
                        metallicity_tng_sat_rel = tng_data_sat_rel[:,7][mask_min]
                        self.metallicity_tng_sat_rel = metallicity_tng_sat_rel
        ...
        if self.include_metallicity:
            self.metallicity_main = metallicity_main
    ...
    def bin_data(self, dlogM=0.1, mhalo_min=0, mhalo_max=20, sfr_max = 5, sfr_min=None, bh_mass_max=20, bh_mass_min=None):
        ...
        for i, mhalo_val in enumerate(mhalo_bins[:-1]):
            ...
            if self.include_metallicity:
                metallicity_binned.append(self.metallicity_main[self.mask_main][mask_bin])
        ...
        self.metallicity_binned = metallicity_binned
        

    def shuffle_in_bins(self, seed=0, do_shuffle=True, do_polyfit=False, odir="/cosma7/data/dp004/dc-zhan5/shuffled", Nsplit=1, split_property="conc", split_test=False, Nsplit2=1, split_property2="bh", metallicity_seed="same"
):
        ...
        if metallicity_seed == "same":
            seed_metallicity = seed
        elif metallicity_seed == "+1":
            seed_metallicity = seed + 1
        else:
            raise ValueError(f"{metallicity_seed} Not valid seed description for metallicity")
        rng1 = np.random.default_rng(seed)  # Generator for the first array
        rng2 = np.random.default_rng(seed_metallicity)  # Generator for the second array

        #random.seed(seed)

        shuffled_sfrs = []
        shuffled_ihalo = []
        shuffled_ihalo_metallicity = []
        ...
        for ibin in range(len(self.mhalo_binned)):
            ...
            if len_sfr_binned > 0:
                ...
                rng1.shuffle(indices) # shuffle order of halos in bin
                #print(indices)
                indices_metallicity = np.arange(0,len(self.sfr_binned[ibin]),1) 
                rng2.shuffle(indices_metallicity) # shuffle order of halos in bin
                ...
                if do_shuffle:
                    if do_polyfit:
                        ...
                        if Nsplit == 1:
                            ...
                            shuffled_metallicity.append(np.array(self.metallicity_binned[ibin])[indices_metallicity]) 
                            shuffled_ihalo_metallicity.append(np.array(self.ihalo_binned[ibin])[indices_metallicity])
                            ...
                            rng1.shuffle(bin_assignments_not_nan)
                                ...
                                if split_test is False:
                                    ...
                                    if Nsplit2 > 1:
                                        ...
                                        for ip2 in range(Nsplit2):
                                            ...
                                            rng1.shuffle(indices_percentiles)
                    ...
                    shuffled_metallicity.append(np.array(self.metallicity_binned[ibin]))   
                    shuffled_ihalo_metallicity.append(np.array(self.ihalo_binned[ibin]))   
            else:
                # If no halos in bin
                shuffled_sfrs.append([])
                shuffled_ihalo.append([])

                shuffled_metallicity.append([])
                shuffled_ihalo_metallicity.append([])
        ...
        if self.object2shuffle == "gal" or self.object2shuffle == "sat":
            ...
            new_sat_metallicity = np.empty_like(self.metallicity_sat_rel)
            index_prev = 0
            ...
            for ibin in range(len(self.mhalo_binned)):
                if len(self.sfr_binned[ibin]) > 0:
                    ...
                    if do_shuffle:
                        ...
                        cent_ihalos_in_bin_metallicity = shuffled_ihalo_metallicity[ibin]
                    else:
                    ...
                    mask_sat_metallicity = np.isin(self.ihalo_sat_rel, cent_ihalos_in_bin_metallicity)
    ...
    # np.searchsorted returns first index in cent_ihalos_in_bin_sorted which corresponds to self.ihalo_sat_rel[mask_sat] 
                        ...
                        # Do similar for metallicity
                        sort_indices = np.argsort(cent_ihalos_in_bin_metallicity)
                        cent_ihalos_in_bin_metallicity_sorted = cent_ihalos_in_bin_metallicity[sort_indices]
                        ...
                        new_sat_metallicity[index_prev:index_next] = self.metallicity_sat_rel[mask_sat]
            ...
            if nsat_shuffle != nsat_tot:
                ...
                new_sat_metallicity[nsat_shuffle:nsat_tot] = self.metallicity_sat_abs[mask_not_shuffled_sats]
        ...
        if self.include_metallicity is True:
            metallicity_dir = "metallicity"
        else:
            metallicity_dir = ""

        odir1 =f"{odir}/logM10_dlogM{self.dlogM}_snap{self.snapnum}/changed_bins/{metallicity_dir}/{self.ps_type}_ps/shuffle_{self.object2shuffle}/{self.mask_main_name}/{split_name}"
        ...
        print("# log10(mass[Msun/h]) x[Mpc/h] y[Mpc/h] z[Mpc/h] log10(sfr[Msun/yr]) log10(metallicity)", file=f_out)
            ...
            for ibin in range(len(self.mhalo_binned)):
                if len(self.sfr_binned[ibin]) > 0:
                    for ihalo in range(len(self.sfr_binned[ibin])):
                        ...
                        if sfr1 > -5:
                            print(self.mhalo_binned[ibin][ihalo], self.pos_binned[ibin][ihalo][0], self.pos_binned[ibin][ihalo][1], self.pos_binned[ibin][ihalo][2], sfr1, self.metallicity_binned[ibin][ihalo], file=f_out)
                            count_cent += 1
        ...
        if self.ps_type == "gal" or self.ps_type == "sat":
            ...
            if self.object2shuffle == "cent":
                ...
                for i in range(len(self.mhalo_sat_abs)):
                    print(self.mhalo_sat_abs[i], self.pos_sat_abs[i][0], self.pos_sat_abs[i][1], self.pos_sat_abs[i][2], self.sfr_sat_abs[i], self.metallicity_sat_abs[i], file=f_out)
            else:
                ...
                for i in range(nsat_tot):
                    ...
                    print(new_sat_mhalo[i], new_sat_pos[i][0], new_sat_pos[i][1], new_sat_pos[i][2], new_sat_sfr[i], new_sat_metallicity[i], file = f_out)
                    count_sat += 1
        ...
        if self.ps_type != "sat":
            ...
            metallicity_tng_min = self.metallicity_main[~self.mask_main]
            for i in range(len(mhalo_tng_min)):
                if sfr_tng_min[i] > -5:
                    print(mhalo_tng_min[i], pos_tng_min[i][0],pos_tng_min[i][1],pos_tng_min[i][2], sfr_tng_min[i], metallicity_tng_min[i],file = f_out)
