...
class Shuffler:
    def __init__(self, ps_type, object2shuffle, snapnum=40, boxsize=205, include_bh=False, include_conc=False, include_msat_sum=False):
    ...
    def read_tng(self):
        ...
        if self.include_msat_sum:
            ...
            arr_all = np.arange(0, max(self.ihalo_main))

            
            msat_sum_all = np.zeros(max(self.ihalo_main)+1)
            msat_sum_all[msat_sum_ihalo] = msat_sum

            #sat_submass_sum_ihalo_all = np.arange(max(self.ihalo_main)+1)
            #sat_submass_sum_ihalo_all[sat_submass_sum_ihalo] = sat_submass_sum_ihalo


            #mask = np.isin(sat_submass_sum_ihalo, self.ihalo_main)

            self.msat_sum = msat_sum_all[self.ihalo_main]
            #self.sat_submass_sum_ihalo = sat_submass_sum_ihalo_all[self.ihalo_main]
            #assert np.array_equal(self.sat_submass_sum_ihalo, self.ihalo_main)
    ...
    def shuffle_in_bins(self, seed=0, do_shuffle=True, do_polyfit=False, odir="/cosma7/data/dp004/dc-zhan5/shuffled", Nsplit=1, split_property="conc", split_test=False
):
        ...
        split_properties = ["conc", "msat_sum"]
    ...
    # np.searchsorted returns first index in cent_ihalos_in_bin_sorted which corresponds to self.ihalo_sat_rel[mask_sat] 
        ...
        if Nsplit == 1:
            ...
            if split_test == False:
                ...
                split_name = f"split_nan/test_split{Nsplit}"


        odir1 =f"{odir}/logM10_dlogM0.1_snap{self.snapnum}/changed_bins/{self.ps_type}_ps/shuffle_{self.object2shuffle}/{self.mask_main_name}/{split_name}"
