...
class Shuffler:
	...
	def shuffle_in_bins(self, seed=0, do_shuffle=True, do_polyfit=False, odir="/cosma7/data/dp004/dc-zhan5/shuffled", split_by_conc=False
):
			...
			if len(self.sfr_binned[ibin]) > 0:
				...
				if do_shuffle:
					if do_polyfit:
						...
						if not split_by_conc:
							shuffled_sfrs.append(np.array(self.sfr_binned[ibin])[indices]) 
							shuffled_ihalo.append(np.array(self.ihalo_binned[ibin])[indices])
						else:
							#conc_median = np.median(np.array(self.conc_binned[ibin]))
							conc_threshold = 0
							conc_high_mask = self.conc_binned[ibin] > conc_threshold
							conc_low_mask = self.conc_binned[ibin] < conc_threshold
							...
							conc_median = np.median(np.array(self.conc_binned[ibin][conc_high_mask]))
							#print("conc_median:", conc_median)
							conc_high_mask_high = conc_high_mask & (self.conc_binned[ibin] > conc_median)
							#print(np.sum(conc_high_mask_high))
							conc_high_mask_low = conc_high_mask & (self.conc_binned[ibin] < conc_median)
							#print(np.sum(conc_high_mask_low))
							
							indices_high_low = indices_orig[conc_high_mask_low]
							random.shuffle(indices_high_low)
							#print("indices_high_low", indices_high_low)
							indices_shuffled[conc_high_mask_low]= indices_high_low
							#print("indices_shuffled_high_low", indices_shuffled)

							indices_high_high = indices_orig[conc_high_mask_high]
							#print("indices_high_high_before_shuffle", indices_high_high)
							random.shuffle(indices_high_high)
							#print("indices_high_high_after_shuffle", indices_high_high)
							indices_shuffled[conc_high_mask_high] = indices_high_high
							#print("indices_shuffled_high_high", indices_shuffled)

													

							indices_low = indices_orig[conc_low_mask]
							random.shuffle(indices_low)
							indices_shuffled[conc_low_mask] = indices_low
	...
	# np.searchsorted returns first index in cent_ihalos_in_bin_sorted which corresponds to self.ihalo_sat_rel[mask_sat] 
		...
		if not split_by_conc:
			...
			conc_split_name = "conc_split_nan/conc_split2"
		...
		if not os.path.exists(odir1):
			try:
				...
				pass
		fname = f"{odir1}/seed{seed}{shuffle_name}_test3.txt"
