...
if halo_type == "gal":
    ...
    if ihalo is False:
        ...
        mhalo_tng3 = tng_data3[:,0]
        pos_tng3 = tng_data3[:,1:4]
        sfr_tng3 = tng_data3[:,4]
        ...
        mhalo_tng3 = tng_data3[:,1]
        pos_tng3 = tng_data3[:,2:5]
        sfr_tng3 = tng_data3[:,5]


    mass_lim = 14
    dist_sat_mask = mhalo_tng1 < mass_lim

    mhalo_tng = np.concatenate((mhalo_tng1[dist_sat_mask], mhalo_tng2[mhalo_tng2 < mass_lim
]))
    pos_tng = np.concatenate((pos_tng1[dist_sat_mask], pos_tng2[mhalo_tng2 < mass_lim]))
    sfr_tng = np.concatenate((sfr_tng1[dist_sat_mask], sfr_tng2[mhalo_tng2 < mass_lim]))

    group_mask = mhalo_tng3 >= mass_lim
    mhalo_tng = np.concatenate((mhalo_tng, mhalo_tng3[group_mask]))
    pos_tng = np.concatenate((pos_tng, pos_tng3[group_mask]))
    sfr_tng = np.concatenate((sfr_tng, sfr_tng3[group_mask]))
    ...
    #mmax = 13.5
    #mlim = 13.6
    #mask = (mhalo_tng1 > mmin) & (mhalo_tng1 < mmax) 
    mask = (mhalo_tng1 > 10) 
    #mask = sfr_tng1 < -3
...
#       sfr_tng[ihalo] = np.random.normal(2.4, 0.2)
    #sfr_const = 2
    #sfr_tng = np.where(mhalo_tng < mlim, sfr_tng, sfr_const)
...
import pickle
#odir=f"ps_data/snap{snapnum}/mvir/const_above_mlim"
...
#fname = f"{odir}/logM{mlim}_logSFR{sfr_const}.pickle"
